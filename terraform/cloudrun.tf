...
resource "google_cloud_run_v2_service" "simple_relay" {
  ...
  template {
    ...
    containers {
      image = "us-central1-docker.pkg.dev/${var.project_id}/${var.service_name}-${var.environment}/${var.service_name}:${var.image_tag}"
...
resource "google_cloud_run_v2_service" "simple_billing" {
  name     = "${var.billing_service_name}-${var.environment}"
  location = var.region
  ...
  template {
    ...
    containers {
      image = "us-central1-docker.pkg.dev/${var.project_id}/${var.service_name}-${var.environment}/${var.billing_service_name}:${var.image_tag}"
